...
%                 't0_lsig' ,1, ...
%                 'l'       ,1);
%             load(['data_res_basedonBayes_simple_v2.mat'])
% 
%         case 3
% 
%             par_info = struct( ...
...
%                 't0_lsig' ,1, ...
%                 'l'       ,1);
%             load(['data_res_basedonBayes_simple_noPast_v2.mat'])
% 
% 
%         case 5
% 
...
%     end
%     par_=par_m(1,:);
%     [nlLH(model) pp] = nlLH_DDM_basedonStaticBayes_new(par_,par_info,data_in,o);
%     BIC_(model) = length(par_)*log(size(pp,3))+2*nlLH(model);
%     BIC_10(model) = length(par_)*log(size(pp,3))/log(10)+2*nlLH(model)/log(10);
% end
...
for model = 1:4
    ...
    % if ismember(model,[1,4])
    %     [nlLH(model) pp] = nlLH_DDM_basedonStaticBayes(par_,par_info,data_in,o);
    % else
    %     [nlLH(model) pp] = nlLH_DDM_basedonStaticBayes_newsimp(par_,par_info,data_in,o);
    % end
    pp = data_in.z;
    BIC_(model) = length(par_)*log(size(pp,3))+2*nlLH(model);
    BIC_10(model) = length(par_)*log(size(pp,3))/log(10)+2*nlLH(model)/log(10);
...
close
bar([1:length(dBIC)],dBIC,'EdgeColor','none')
% bar([1:5],dLH,'EdgeColor','none')
text([1:length(dBIC)],ones(size(dBIC))*1750,string(round(dBIC)),'HorizontalAlignment','center')
% axis([.5 5.5 0 1000])
d = .5
% axis([1-d length(dBIC)+d 0 ceil(max(dBIC/10)*1.1)*10])
...
xticklabels({'complex','simple with AP','simple with RV','only STSE'})
box off
