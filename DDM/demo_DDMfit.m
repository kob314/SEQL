...
switch model
    ...
    % load(['res_basedonBayes_numpyro.mat']);
    % load(['res_basedonBayes_numpyro_withoutrescale.mat']);
    % load(['res_basedonBayes_numpyro_withoutrescale_.mat']);

    % load(['res_basedonBayes_numpyro_withrescale_.mat']);
    % load(['res_basedonBayes_numpyro_withrescale_old.mat']);

    % load(['res_basedonBayes_numpyro_withoutrescale_subjAP.mat']) 
    
    par_=par_m(1,:);
    par_s = parameter_wrap(par_,par_info,'v2s');
%     par_=[0.4981    3.7545    0.2613    0.0044    0.0748    0.6426   14.1470    1.0901    0.6159    0.1994    0.0622];
    % [nlLH pp] = nlLH_DDM_basedonStaticBayes(par_,par_info,data_in,o);  
...
% par_x=par_; par_x(2) = 0; par_x(5:6) = 0;
% [nlLH ppw] = nlLH_DDM_basedonStaticBayes(par_x,par_info,data_in,o);
% 
% par_x=par_; par_x(1) = 0; par_x(5:6) = 0;
% [nlLH ppv] = nlLH_DDM_basedonStaticBayes(par_x,par_info,data_in,o);
% 
% par_x=par_; par_x(1:2) = 0;
% [nlLH pps] = nlLH_DDM_basedonStaticBayes(par_x,par_info,data_in,o);
